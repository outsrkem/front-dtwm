    ...
    <div>
        ...
        <div class="card">
            <div v-if="!result" v-loading="basic.loading">
                ...
                <van-popup v-model:show="selectItem.dialogVisible" :style="{ padding: '18px' }" closeable position="bottom">
                    <el-checkbox-group v-model="selectedRows">
                        <div style="width: 100%" v-for="(item, inx) in items">
                            <el-checkbox :label="item.name" :value="item" />
                        </div>
                    </el-checkbox-group>
                    <div style="display: flex; justify-content: center; padding: 10px">
                        <el-button style="width: 130px" bg text @click="onCloseItem">取消</el-button>
                        <el-button style="width: 130px" type="primary" @click="onAddItemTolist()">确定</el-button>
                    </div>
                </van-popup>
                <!-- <el-drawer v-model="selectItem.dialogVisible" direction="btt" size="600">
                    <el-checkbox-group v-model="selectedRows">
                        <div style="width: 100%" v-for="(item, inx) in items">
                            <el-checkbox :label="item.name" :value="item" />
                        </div>
                ...
                <!-- 物品数目 -->
                <van-cell-group>
                    <van-cell v-for="(val, inx) in basic.items" :key="inx" :title="val.name">
                        <el-space>
                            <van-button icon="minus" type="danger" size="mini" round @click="onRemoveItemlist(inx)" />
                            <input
                                type="text"
                                v-model="basic.items[inx].quantity"
                                style="padding: 0; width: 60px; outline: none; border: none; background: none"
                                placeholder="入库数目" />
                        </el-space>
                    </van-cell>
                </van-cell-group>
...
export default {
    ...
    components: { pagination, MyHeader },
    props: {},
    setup() {
        return {
            Plus,
